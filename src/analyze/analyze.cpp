...
void Analyze::coerce_value_to_col(Value &value, const ColMeta &col) {
    if (value.type == col.type) {
        return;
    }
    if (col.type == TYPE_DATETIME && value.type == TYPE_STRING) {
        int64_t datetime_val = 0;
        if (!parse_datetime_value(value.str_val, datetime_val)) {
    ...
    if (col.type == TYPE_FLOAT && value.type == TYPE_BIGINT) {
...
Value Analyze::convert_sv_value(const std::shared_ptr<ast::Value> &sv_val) {
    ...
    if (auto int_lit = std::dynamic_pointer_cast<ast::IntLit>(sv_val)) {
        ...
        try {
            size_t parsed_len = 0;
            parsed = std::stoll(int_lit->val, &parsed_len);
            if (parsed_len != int_lit->val.size()) {
                throw InternalError("Invalid integer literal");
            }
